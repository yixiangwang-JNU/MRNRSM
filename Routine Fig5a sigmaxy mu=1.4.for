*****************************************************
*     Calculate sigmaxy of a nodal-ring semimetal 
*****************************************************
      module value
      implicit double precision (a-h,o-z)      
      real(8), parameter :: Pi=3.14159 
      real(8), parameter :: xMbar=0.53          ! Dirac mass
      real(8), parameter :: gamma=0.7           ! dimensionless Fermi velocity 
      real(8), parameter :: Upper=2             ! upper limit in the summation       
      integer, parameter :: Lz=1000             ! size in the z direction 
      real(8), parameter :: dkz=2*Pi/Lz         ! precision
      complex(8), parameter :: Im=(0,1.d0)     
cc     
      real(8) sigmaxy(1600),etaArray(6)
      complex(8) cni1,cnj1 
      common cni1,cni2,cnj1,cnj2,
     &       Binv,xB,xmu,eta, 
     &       eni,enj,  
     &       ni,nj,is,js,Ncut                    ! index for the initial and final LLs  
      end module value
cc      
**********************
*     main program
**********************
      program main
      use value
      implicit double precision (a-h,o-z)
      integer, parameter :: Num=931 
      real(8) Array(Num,3)                      ! store inverseB and chemical potential  
      character(100) filename
      open(10,file='mu=1.4Blog.dat') 
cc
      etaArray(1)=0.01
      etaArray(2)=0.02
      etaArray(3)=0.05
      etaArray(4)=0.1 
      etaArray(5)=0.2
      etaArray(6)=0.5
cc             
      do i=1,Num
         read(10,*) Array(i,1),Array(i,2),Array(i,3)     ! read chemical potential and Ncut
      end do      
cc
      do 101 j=1,6
      eta=etaArray(j) 
      write(filename,'(a13,f8.4,a4)') 'sigmaxy,eta=',eta,'.dat' 
      open(20,file=filename) 
cc
      do 100 i=1,Num                            ! number of magnetic field 
         xB=Array(i,1) 
         xmu=Array(i,2)
         Ncut=Array(i,3)
cc
cc       transition: n,s->n+1,s'  
         sigmaxy=0 
         do ni=0,Ncut+400                        ! sum from the zeroth LL  
            nj=ni+1
            call solve_fn(Res)
            sigmaxy(ni+1)=Res 
         end do    
cc
         sum=0
         do ni=0,Ncut+400
            sum=sum+sigmaxy(ni+1) 
         end do
         write(20,'(100f16.8)') xB,sum 
cc 
100   continue     
101   continue 
cc      
      end program main
cc
*****************************
*     subroutine solve_fn
*****************************
      subroutine solve_fn(Res) 
	use value
      implicit double precision (a-h,o-z)
cc   
      Res=0
      do 100 is=-1,1,2
      do 200 js=-1,1,2
         do 300 zk=1d-6,Upper,dkz
            call solve_ni(zk)
            call solve_nj(zk) 
cc 
            if(eni<xmu .and. enj>xmu) then
               call fn(Resn) 
               Res=Res+Resn*4*xB*xB                   ! note the coefficient 2 
            end if
cc
            if(eni>xmu .and. enj<xmu) then
               call fn(Resn) 
               Res=Res-Resn*4*xB*xB 
            end if               
300      continue  
200   continue
100   continue
      Res=Res*2/Lz
cc      
      end subroutine solve_fn
cc      
************************
*     subroutine: fn
************************
      subroutine fn(Resn)
      use value
      implicit double precision (a-h,o-z)  
      complex(8) temp1 
cc 
      Resn=0 
      temp1=cni2*cnj1+conjg(cni1)*cnj2      ! note the coefficient 
      temp2=(ni+1)*abs(temp1)**2       
      temp3=(eni-enj)**2-eta**2
      temp4=((eni-enj)**2+eta**2)**2 
      Resn=temp2*temp3/temp4
cc
      end subroutine fn      
cc 
*********************************************
*     subroutine: solve the initial ni LL
********************************************* 
      subroutine solve_ni(zk)
      use value
      implicit double precision (a-h,o-z)      
cc
      aa=(2*ni*xB+xB-1)**2+xMbar**2+(gamma*zk)**2
      en=sqrt(aa)
      eni=is*en 
cc
      cni1=2*ni*xB+xB-1-Im*gamma*zk
      cni2=-(xMbar+is*en)
      bb=sqrt(abs(cni1)**2+cni2**2) 
      cni1=cni1/bb
      cni2=cni2/bb       
cc      
      end subroutine solve_ni      
cc       
********************************************
*     subroutine: solve the final nj LL
******************************************** 
      subroutine solve_nj(zk)
      use value
      implicit double precision (a-h,o-z)      
cc
      aa=(2*nj*xB+xB-1)**2+xMbar**2+(gamma*zk)**2
      en=sqrt(aa)
      enj=js*en      
cc
      cnj1=2*nj*xB+xB-1-Im*gamma*zk
      cnj2=-(xMbar+js*en)
      bb=sqrt(abs(cnj1)**2+cnj2**2) 
      cnj1=cnj1/bb
      cnj2=cnj2/bb       
cc      
      end subroutine solve_nj
cc